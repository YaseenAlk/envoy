...
class BufWrapper(ProtoBreakingChangeDetector):
    ...
    def _run_buf(path_to_before, path_to_after, additional_args=None):
        ...
        initial_code, initial_out, initial_err = run_command(' '.join([buf_path, f"build -o {Path(temp_dir.name, BUF_LOCK_FILE)}", *buf_args]))
        ...
        final_code, final_out, final_err = run_command(' '.join([buf_path, f"breaking --against {Path(temp_dir.name, BUF_LOCK_FILE)}", *buf_args]))
    ...
    def lock_file_changed(path_to_before, path_to_after, additional_args=None):
        _, _, initial_lock, final_lock = BufWrapper._run_buf(path_to_before, path_to_after, additional_args)
